...
export default function Plans({ onSelectPlan }: PlansProps) {
  ...
  const getPlanIcon = (id: string) => {
    switch (id) {
      ...
      case 'protocolo':
        return <Zap className="w-5 h-5 text-red-500 sm:w-6 sm:h-6" />;
      default:
        return <User className="w-5 h-5 text-red-500 sm:w-6 sm:h-6" />;
    }
  };
  ...
  return (
    <section id="plans" className="relative py-24 bg-zinc-950 overflow-hidden border-t border-white/5">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <div className="flex items-center justify-center gap-2 max-w-md mx-auto">
            ...
            <p className="text-[10px] sm:text-xs text-zinc-400 font-bold uppercase tracking-widest text-center whitespace-nowrap">
              ACOMPANHAMENTO COMPLETO PARA <span className="text-red-500">RESULTADOS REAIS</span>
            </p>
            <div className="h-[1px] bg-red-600/30 flex-1" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start max-w-5xl mx-auto">
          ...
          <div className="lg:col-span-8 space-y-3.5 w-full">
            {PLANS.map((plan) => {
              ...
              const isProtocolo = plan.id === 'protocolo';
              return (
                <button
                  key={plan.id}
                  ...
                  className={`w-full text-left flex items-center justify-between p-4 sm:p-5 transition-all duration-300 relative group cursor-pointer border ${
                    isTrimestral
                      ...
                      : isProtocolo
                      ? 'bg-zinc-900/95 border-red-600 shadow-[0_0_22px_rgba(220,38,38,0.18)] hover:border-red-500'
                      : 'bg-zinc-900 border-white/5 hover:border-red-600/30 hover:bg-zinc-900/90'
                  }`}
                  style={{ borderRadius: '12px' }}
                >
                  {/* Glowing background animation on hover */}
                  <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none rounded-[12px] ${
                    isTrimestral 
                      ? 'bg-gradient-to-r from-amber-500/[0.02] to-amber-500/[0.05]' 
                      : isProtocolo
                      ? 'bg-gradient-to-r from-red-600/[0.04] to-red-600/[0.08]'
                      : 'bg-gradient-to-r from-red-600/[0.01] to-red-600/[0.04]'
                  }`} />

                  {/* Star golden badge for Trimestral */}
                  {isTrimestral && (
                    <div className="absolute -top-3 left-6 bg-gradient-to-r from-amber-500 to-yellow-400 text-black text-[9px] font-black uppercase tracking-widest px-3 py-1 shadow-md flex items-center gap-1 z-10" style={{ borderRadius: '4px' }}>
                      <Sparkles className="w-3 h-3 text-black animate-pulse" />
                      PLANO MAIS RECOMENDADO
                    </div>
                  )}

                  {/* Red pulsing badge for Protocolo */}
                  {isProtocolo && (
                    <div className="absolute -top-3 left-6 bg-gradient-to-r from-red-600 to-red-500 text-white text-[9px] font-black uppercase tracking-widest px-3 py-1 shadow-md flex items-center gap-1 border border-red-500/30 z-10" style={{ borderRadius: '4px' }}>
                      <Zap className="w-3 h-3 text-white animate-bounce shrink-0" />
                      PROTOCOLO INDEPENDENTE — APENAS PROTOCOLO & SUPLEMENTAÇÃO
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-3.5 sm:gap-5 flex-1 pr-4">
                    ...
                    <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center border shrink-0 transition-transform duration-300 group-hover:scale-105 ${
                      isTrimestral
                    ...
                    <div className="flex flex-col justify-center flex-1">
                      <span className={`text-base sm:text-lg font-black uppercase tracking-wider transition-colors duration-300 ${
                        isTrimestral 
                        ...
                        {plan.shortName || (plan.name.split(' ')[1] || plan.name)}
                      </span>
                      <span className="text-[9px] sm:text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-0.5">
                        {plan.durationLabel || `${plan.months} ${plan.months === 1 ? 'MÊS' : 'MESES'} DE ACOMPANHAMENTO`}
                      </span>

                      {/* Clarification Box for Protocolo to address doubts */}
                      {isProtocolo && (
                  ...
                  <div className="text-right flex items-center gap-3 shrink-0">
                    <div className="flex items-baseline">
                      <span className="text-[10px] sm:text-xs font-black text-red-500 mr-1">R$</span>
                      ...
                      <span className="text-xs sm:text-sm font-black text-zinc-500 group-hover:text-zinc-400">,00</span>
                    </div>
            ...
            <div className="border border-white/5 bg-zinc-900/50 p-6 space-y-4 shadow-xl" style={{ borderRadius: '12px' }}>
              <div className="flex items-center justify-center gap-2">
                <div className="h-[1px] bg-white/5 flex-1" />
                <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">FORMAS DE PAGAMENTO</span>
                <div className="h-[1px] bg-white/5 flex-1" />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center pt-2">
                ...
                <div className="flex items-center gap-2.5 justify-center sm:justify-start">
                  ...
                  <div className="text-left">
                    <span className="block text-[10px] font-black uppercase tracking-wider text-white">PIX</span>
                    <span className="text-[8px] text-zinc-500 block uppercase font-bold">PAGAMENTO À VISTA</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 justify-center">
                  ...
                  <div className="text-left">
                    <span className="block text-[10px] font-black uppercase tracking-wider text-white">PARCELADO NO CARTÃO</span>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="text-[7.5px] bg-[#00a6ca]/10 text-[#00a6ca] border border-[#00a6ca]/20 px-1 py-0.2 rounded font-black tracking-wide uppercase">MERCADO PAGO</span>
                      <span className="text-[7.5px] bg-[#34d399]/10 text-[#34d399] border border-[#34d399]/20 px-1 py-0.2 rounded font-black tracking-wide uppercase">INFINITEPAY</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 justify-center sm:justify-end">
                  ...
                  <div className="text-left sm:text-right">
                    <span className="block text-[10px] font-black uppercase tracking-wider text-white">PAGAMENTO RECORRENTE</span>
              ...
              <div className="border-t border-white/5 pt-3.5 flex items-center justify-center gap-2 text-center">
                ...
                <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider">
                  PARCELAMENTO DISPONÍVEL APENAS COM AS <span className="text-amber-500">TAXAS DAS OPERADORAS</span>.
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-4 w-full">
            ...
            <div className="bg-zinc-900 border border-white/5 p-6 space-y-5" style={{ borderRadius: '12px' }}>
              <div>
            ...
            <div className="bg-zinc-900 border border-red-600/20 p-6 space-y-4 relative overflow-hidden group" style={{ borderRadius: '12px' }}>
              <div className="absolute top-0 right-0 w-16 h-16 bg-red-600/[0.04] rounded-full blur-xl" />
              
              <div className="flex items-center gap-2">
                <Globe className="w-4 h-4 text-red-500" />
                <span className="text-[9px] font-black text-red-500 uppercase tracking-widest">ACOMPANHAMENTO GLOBAL</span>
              </div>

              <div className="space-y-1">
                <h5 className="text-sm font-black text-white uppercase tracking-tight">CONSULTORIA 100% ONLINE</h5>
                <p className="text-[10px] text-zinc-400 font-semibold leading-relaxed uppercase">
                  Atendimento para qualquer lugar do Brasil e do mundo. Nosso suporte e planos são totalmente digitais, direto no seu WhatsApp.
                </p>
              </div>

              <div className="pt-2 space-y-2 border-t border-white/5">
                <div className="flex items-center gap-2 text-[9px] font-bold text-zinc-300 uppercase tracking-wider">
                  <span className="text-red-500">✓</span> Treino e Dieta na tela do seu celular
                </div>
                <div className="flex items-center gap-2 text-[9px] font-bold text-zinc-300 uppercase tracking-wider">
                  <span className="text-red-500">✓</span> Suporte constante direto com o Bruno
                </div>
                <div className="flex items-center gap-2 text-[9px] font-bold text-zinc-300 uppercase tracking-wider">
                  <span className="text-red-500">✓</span> Sem necessidade de deslocamento físico
                </div>
              </div>
            </div>
